...
export async function credentialPutRepository(userId:  number, credentialId: number,  credential: PutCredential){
    ...
    try {
       ...
       // console.log("existingCredential", existingCredential)
        ...
        const verifyOwnerCredential = await prisma.credentials.findFirst({
            where: {
                id: credentialId
            }
        })
        console.log("verify", verifyOwnerCredential)

        if(verifyOwnerCredential.userId !== userId) throw { type: "not found", message: "Credencial sem permissao" };
